...
- **`/admin/live-monitor` interactieve kaart** voor admin-only monitoring. Server-side klusdata via Prisma + geocoding via PDOK (`postcodeNaarCoords`, cached 7 dagen) — coördinaten verlaten de server pas met de marker-data, niet zichtbaar voor non-admins. Drie statussen: 🔵 Nieuw (geen leads), 🟠 Chat actief (bericht in laatste 7 dagen), 🟡 Wachtend (lead gekocht zonder recente chat). Kaart via `react-leaflet` + OpenStreetMap-tiles, dynamic import met `ssr:false` om SSR-crash te vermijden. Gesplitst in `LiveKaart` (wrapper) + `LiveKaartInner` (echte leaflet-code). Sidebar krijgt "Live monitor"-link tussen Showcase en Activity feed. **TODO:** "Recent afgehandeld"-status (groen) vereist een `gesloten_op DateTime?` veld op `Klus` — toekomstige migratie.
